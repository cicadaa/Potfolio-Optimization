...
POSITIVE VARIABLES
         ...
         sell(i)
         InitHold(i)
         borrow          'Amount borrowed at the borrowing rate'         
;
...
BudgetCon ..             sum(i, x(i)) =E= Budget + borrow;

InCon ..                 sum(i, x(i)) =E= SUM(i,InitHold.l(i)) +borrow;
...
ReturnDefWithCost..      PortReturn =e= SUM(i, ( EP(i)*x_0(i) - FlatCost*Y(i) ) ) +
                          SUM(i, (EP(i) - PropCost)*x_1(i))- (borrow * BorrowRate) ;
...
BalanceCon(i)..           x(i) - buy(i) + sell(i) =e=  InitHold.l(i) - borrow.l ;
...
loop(p$(ord(p)>1 and ord(p)<3),   
    ...
    borrow.UP = 2.0*SUM(i,InitHold.l(i));
*    Budget = SUM(i,InitHold.l(i));
    ...
    CVaRMAX(p) = CVaR.l;
    DISPLAY sell.l,buy.l,x.l,borrow.l,ExpRetLim;
    );
